...
#     print(f"Offsets mapping: {offsets}")
#     start_char = text.find(substring)
#     print(f"Finding substring '{substring}' in text '{text}' at char position {start_char}")
#     if start_char == -1:
#         return None  # 没找到该子串
...
#     print(f"Mapped to token positions: start={token_start}, end={token_end}")
...
# def find_tokens(tokenizer, text, substring, last=True):
#     token_range = find_tokens_by_char(tokenizer, text, substring)
#     if token_range is None:
#         return None
#     start, end = token_range
#     return end if last else (start, end)
...
def get_hidden_states(model, tokenizer, entity, prompt):
    ...
    # entity_idx = [find_tokens(tokenizer, q, e, prepend_space) for q, e in zip(prompt, entity)]
